...
## 2026-01-29 - Zombie Token Authorization Bypass
**Vulnerability:** The `authenticateUser` middleware was using a "fail open" logic where if a user record was not found in the database (e.g., deleted user), the code fell through to allowing the request using the token payload (`req.user = decoded`). This allowed deleted users with valid JWTs to access the system until token expiration.
**Learning:** Authentication middleware must explicitly validate the existence of the user record in the database, not just the validity of the JWT signature. Do not assume `findByPk` always returns a record.
**Prevention:** Always check `if (!userRecord)` and return `401 Unauthorized` immediately if the user is missing.
