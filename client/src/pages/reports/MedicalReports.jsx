...
const MedicalReports = () => {
  ...
  const [selectedReportForSamples, setSelectedReportForSamples] =
    useState(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [reportsRes, patientsRes, antibioticsRes] = await Promise.all([
        ...
        axios
          .get(`${apiUrl}/antibiotics`, { headers })
          .catch((e) => ({ data: [] })),
      ]);
    ...
    async (reportId) => {
      try {
        ...
        const response = await axios.get(
          `${apiUrl}/medical-reports/${reportId}/comments`,
          { headers },
        );

        // Map test comments and their images
        const testComments = (response.data.testComments || []).map(
          (comment) => ({
            ...comment,
  ...
  const deleteTestComment = async (commentId) => {
    try {
      ...
      await axios.delete(
        `${apiUrl}/medical-reports/test-comments/${commentId}`,
        { headers },
      );
    ...
    if (newImages.length < oldImages.length) {
      const removedImage = oldImages.find((oldImg) => {
        ...
        return !newImages.some(
          (newImg) => newImg.image_name === oldImg.image_name,
        );
      });
  ...
  const formatCellData = (value, header, row) => {
    if (
      ...
      return value || 0;
    } else if (header === "tests_count" || header === "cultures_count") {
      return value || 0;
      ...
      tests.forEach((test) => {
        ...
        if (test.results) {
          ...
          Object.entries(test.results).forEach(([key, data]) => {
            if (key === "result" || key === "status" || key === "clinical_flag")
              return;
  ...
  const handleSaveResults = async () => {
    ...
    try {
      ...
      (resultsData.test_results || []).forEach((tr) => {
        ...
        const existingReq = saveRequests.find(
          (req) => req.testId === tr.test_id,
        );
        if (
          !existingReq &&
          tr.result !== undefined &&
  ...
  const handleAiExtraction = async (file) => {
    ...
    try {
      ...
      const expectedKeys = selectedReportForResults.tests.flatMap((test) => {
        ...
        const componentKeys = (testComponents[test.id] || []).map(
          (c) => c.label || c.name,
        );
        ...
        Object.entries(extractedData).forEach(([aiKey, aiValue]) => {
          ...
          selectedReportForResults.tests.forEach((test) => {
            ...
            if (normalizedTestName === normalizedAiKey) {
              const existingIndex = newTestResults.findIndex(
                (tr) => tr.test_id === test.id,
              );
              if (existingIndex !== -1) {
                newTestResults[existingIndex] = {
                  ...newTestResults[existingIndex],
                  result: aiValue,
                };
              } else {
            ...
            let param = structureConfig.find((p) => {
              ...
              return (
                normKey === normalizedAiKey ||
                normName === normalizedAiKey ||
                normLabel === normalizedAiKey ||
                (normalizedAiKey.length > 3 &&
                  normLabel &&
                  (normLabel.includes(normalizedAiKey) ||
                    normalizedAiKey.includes(normLabel)))
              );
            });
            ...
            const component = comps.find((c) => {
              ...
              return (
                normName === normalizedAiKey || normLabel === normalizedAiKey
              );
            });
      ...
      if (matchCount > 0) {
        ...
        toast.update(toastId, {
          render:
            "AI finished scanning but couldn't find matches for the tests in this report.",
          type: "warning",
          autoClose: 5000,
        });
      }
  ...
  const updateTestResult = (testId, result) => {
    setResultsData((prev) => {
      const existingIndex = prev.test_results.findIndex(
        (tr) => tr.test_id === testId,
      );
      let newTestResults;
  ...
  const handleSendWhatsapp = async (reportData) => {
    try {
      ...
      console.log(
        `[Frontend] Sending WhatsApp report for ${reportData.id} to ${reportData.patient.phone}`,
      );
      ...
      if (response.data.success) {
        toast.success(
          response.data.message || "Report sent successfully via WhatsApp!",
        );
        ...
        throw new Error(
          response.data.message ||
            "Backend processed request but delivery status is unknown.",
        );
      }
    } catch (error) {
  ...
  const ActionComponent = ({ rowData }) => {
    return (
      <div className="d-flex gap-1 justify-content-center">
        ...
        <Button
          variant={rowData.done === 1 ? "outline-secondary" : "outline-success"}
          className="action-btn-fixed"
        ...
        <Button
          variant={rowData.collected_at ? "outline-secondary" : "outline-info"}
          className="action-btn-fixed"
          onClick={() => handleMarkCollected(rowData)}
          title={
            rowData.collected_at ? "Mark as Uncollected" : "Mark as Collected"
          }
        ...
        <Button
          ...
          onClick={() => {
            confirm.delete(
              `Report for ${rowData.patient?.name || "unknown"}`,
  ...
  const handleImportXLSX = async (e) => {
    ...
    const loadingToast = toast.loading("Importing medical reports...");
    try {
      const token = localStorage.getItem("token");
      const response = await axios.post(
      ...
      toast.dismiss(loadingToast);
      toast.error(
        error.response?.data?.error || "Failed to import medical reports",
      );
    } finally {
        ...
        {loading ? (
          ...
          <>
            <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap">
              ...
              <div className="d-flex gap-2 flex-wrap">
                ...
                >
                  Unsigned:{" "}
                  {
                    reports.filter((r) => r.done === 0 && r.pending === 0)
                      .length
                  }
                </Badge>
              </div>
                  ...
                  <div>
                    ...
                    <div className="scrollable-tabs">
                      ...
                      >
                            ...
                            selectedReportForResults.tests.length > 0 && (
                              <div className="mb-4">
                                ...
                                {(() => {
                                  ...
                                  const componentResultsMaps = Object.entries(
                                    resultsData.test_component_results || {},
                                  ).reduce((acc, [testId, components]) => {
                                    acc[testId] = Object.entries(
                                      components || {},
                                    ).reduce((cAcc, [compId, data]) => {
                                  ...
                                  return selectedReportForResults.tests.map(
                                    (test, testIndex) => {
                                      ...
                                      const hasStructureConfig =
                                        Array.isArray(test.structure_config) &&
                                        test.structure_config.length > 0;
                                          ...
                                          {hasStructureConfig ? (
                                            <div className="mt-3">
                                              <DynamicResultForm
                                                ...
                                                value={
                                                  componentResultsMaps[
                                                    test.id
                                                  ] || {}
                                                }
                                                onChange={(flatResults) => {
                                                  handleDynamicResultChange(
                                                    test.id,
                                                    flatResults,
                                                  );
                                                }}
                        ...
                        <Tab
                          eventKey="medical-report-images"
                          title="Medical Report Images"
                        >
                          <div className="mt-3">
                            <div className="card">
            ...
            >
              <Modal.Header
                closeButton
                className="bg-white border-0 pb-0 text-dark"
              >
