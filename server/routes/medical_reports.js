...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Optimized to use subqueries for counts instead of fetching all associated objects
          ...
          include: [
            [
              db.sequelize.literal(`(
                SELECT COUNT(*)
                FROM ${db.medical_report_has_test.tableName} AS t
            ...
            [
              db.sequelize.literal(`(
                SELECT COUNT(*)
                FROM ${db.medical_report_has_culture.tableName} AS c
            ...
            [
              db.sequelize.literal(`(
                SELECT COUNT(*)
                FROM ${db.medical_report_has_tg.tableName} AS tg
      ...
      // Add patient_name to each report for easier access
      const reportsWithPatientName = reports.map((report) => {
        ...
        return {
          ...
          tests: [], // Empty array for backward compatibility
          cultures: [], // Empty array for backward compatibility
