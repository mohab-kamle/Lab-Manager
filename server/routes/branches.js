  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        const address = row[headers.indexOf('Address')] || row[1] || null;
        const landline = row[headers.indexOf('Landline')] || row[2] || null;
        const branchNumber = row[headers.indexOf('Branch Number')] || row[3] || null;
        const labName = row[headers.indexOf('Lab')] || row[4];
