...
router.patch("/:id/add-test", authenticateUser, authorizeRoles("admin", "receptionist"), tenantContext, invalidateInvoicesList, async (req, res) => {
    ...
    try {
        ...
        if (newTests.length === 0) {
            await transaction.rollback();
            return res.status(400).json({ error: "No new tests to add or all tests already exist in this invoice." });
        }
        ...
        let medicalReport = await medical_report.findOne({ where: { bill_id: id }, transaction });
        if (!medicalReport) {
            // Create if missing
            medicalReport = await medical_report.create({
                lab_id: lab_id,
