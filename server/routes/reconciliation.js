...
router.post("/", authenticateUser, authorizeRoles("admin", "receptionist"), tenantContext, async (req, res) => {
    ...
    try {
        const { patient_id, amount, payment_method_id, notes, strategy, invoice_ids } = req.body;
        ...
        for (const currentBill of targetBills) {
            ...
            await db.financial_transaction.create({
                amount: amountToApply,
                process_type: 'Due', 
                from: patientRecord.name, 
                to: labRecord.name,
                processed_by_id: req.user.id, // Records which admin/receptionist processed this
                patient_id: patient_id,
                bill_id: currentBill.id, // Explicitly linking the chunk of money to the bill
                payment_method_id: payment_method_id,
                lab_id: req.tenant.lab_id
                // Note: branch_id can be added here if req.tenant/req.user provides it
            }, { transaction });
        ...
        return res.status(201).json({ message: "Reconciliation successful", reconciliation: newReconciliation });

    } catch (error) {
        await transaction.rollback();
        console.error("Error processing reconciliation:", error);
        res.status(500).json({ error: "Failed to process reconciliation", message: error.message });
    }
});
